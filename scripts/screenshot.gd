...
export(String, DIR) var output_path = "res://screenshot folder/"
...
func make_screenshot():
	...
	_update_tags()
	Main.allphotos.append(image)
#	image.save_png("%s%s%s_%s.png" % [output_path, file_prefix, _tag, _index])
	$"/root/Game"._show_controls()
	$"/root/Game".took_picture()
